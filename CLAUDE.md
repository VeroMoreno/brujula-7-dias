...
- ⏭️ **IA-1** — Wrapper Ollama + prompt. `server/lib/ai.js` con `generateSummary(entries, {locale})`, separando `buildSummaryPrompt()` (pura, testeable) de `callOllama()` (fetch a `localhost:11434`, inyectable para mockear). Prompt versionado en `server/prompts/summary.{es,en}.md` — patrones, no consejos.
- ⏭️ **IA-2** — Persistencia del resumen. `writeSummary` / `readSummary`, `data/summary.md` con frontmatter (`created`, `model`, `locale`). Tests round-trip + caso "no existe".
