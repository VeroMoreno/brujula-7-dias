...
- ✅ **IA-3** — Endpoints. `server/routes/summary.js`: `GET /api/summary` (persistido o 404), `POST /api/summary` con guard de jornada completa (409 si no), genera sobre las entradas vía provider inyectable (default `generateSummary` real), persiste y devuelve 201. Locale no soportado → 400, fallo del provider → 502. Montado en `createApp` cuando hay `dataDir`+`promptsDir`. `DEFAULT_MODEL` exportado desde `ai.js`. Tests `tests/summary.api.test.js` mockean el provider (CI sin Ollama).
